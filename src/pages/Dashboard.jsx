...
export default function Dashboard({
  ...
  useEffect(() => {
    ...
    const loadHistory = async () => {
      try {
        ...
        if (!userId) return;
        const data = await generationsListCached(userId, 60000);
        if (!alive) return;
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setHistory(prev => [{ id: generationId, name: form.topic, status: "running" }, ...prev]);
      
      const streamResult = await api.generate.stream(promptText);
      const jsonText = streamResult?.markdown || '';
      ...
      <main className="flex-1 flex flex-col xl:flex-row gap-4 md:gap-6 overflow-hidden">
        ...
        <section className="flex-1 p-14 bg-white/90 dark:bg-zinc-900/90 backdrop-blur-3xl rounded-[40px] shadow-2xl border border-white/20 overflow-y-auto">
          ...
          <button
            ...
            onClick={() => {
              if (!activeId) return;
              window.location.href = api.generations.exportDocx(activeId);
            }}
