...
import SortItOutGame from '../components/SortItOutGame';
import { getDeckSortRound } from '../data/subjectDecks';
import api from '../api';

const T = {
  RU: { title: 'РАЗЛОЖИ ПО ПОЛОЧКАМ', pick: 'Выбери предмет', back: 'Назад' },
  KZ: { title: 'ОРНЫНА ҚОЙ', pick: 'Пәнді таңда', back: 'Артқа' },
  EN: { title: 'SORT IT OUT', pick: 'Pick a subject', back: 'Back' },
};

export default function SortItOutPage({ lang, setLang, user, setUser, grantAchievement, ...accessProps }) {
  const t = T[lang] || T.RU;
  ...
  const handleFinish = async (score, meta) => {
    if (meta.perfect) grantAchievement?.('sorter_supreme');
    api.gameScores.save('sort_it_out', score, { subject, ...meta }).catch(() => {});
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] dark:bg-[#020617] text-slate-900 dark:text-white font-sans pt-[100px] lg:pt-[120px] pb-20">
      ...
      <main id="main-content" className="max-w-2xl mx-auto px-6">
        ...
        {round ? (
          <SortItOutGame round={round} lang={lang} onFinish={handleFinish} onExit={reset} onReplay={() => start(subject)} />
        ) : (
          <>
