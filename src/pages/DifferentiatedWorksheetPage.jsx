...
const T = {
  RU: {
    title: 'РАЗНОУРОВНЕВЫЙ ЛИСТ',
    subject: 'Предмет', topic: 'Тема', grade: 'Класс (базовый уровень)',
  ...
  KZ: {
    title: 'ДЕҢГЕЙЛІК ПАРАҚ',
    subject: 'Пән', topic: 'Тақырып', grade: 'Сынып (негізгі деңгей)',
  ...
  EN: {
    ...
    subject: 'Subject', topic: 'Topic', grade: 'Grade (baseline level)',
...
function buildPrompt(lang, form) {
  ...
  return `You are a helpful teacher assistant. Write a differentiated practice worksheet in ${langName} on the topic "${form.topic}" (subject: ${form.subject}), for a class centered on grade ${form.grade}, covering the SAME topic at three difficulty tiers so a mixed-ability class can work in parallel.

Format:
## Worksheet: ${form.topic}

### Tier 1 — Below grade level (simplified, more scaffolding)
[5-6 questions]

### Tier 2 — At grade level
[5-6 questions]

### Tier 3 — Above grade level (extension/challenge)
[5-6 questions]

## Answer Key
[answers for all three tiers, grouped by tier]

Keep each tier genuinely different in difficulty, not just reworded. Write ONLY the worksheet, no explanations.`;
}

export default function DifferentiatedWorksheetPage({ lang, setLang, user, setUser, ...accessProps }) {
  const t = T[lang] || T.RU;
  const navigate = useNavigate();

  const [form, setForm] = useState({ subject: '', topic: '', grade: 5 });
