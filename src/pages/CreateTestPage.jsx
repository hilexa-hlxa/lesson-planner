...
const QUIZ_MAP_KEY = 'lp_generation_quiz_map';
...
const CreateTestPage = ({ lang, promptConfig, grantAchievement, ...accessProps }) => {
  ...
  const saveQuizMapping = (generationId, quizId) => {
    if (!generationId || !quizId) return;
    try {
      const raw = localStorage.getItem(QUIZ_MAP_KEY);
      const map = raw ? JSON.parse(raw) : {};
      map[generationId] = quizId;
      localStorage.setItem(QUIZ_MAP_KEY, JSON.stringify(map));
    } catch (e) {
      console.error(e);
    }
  };

  const readQuizMapping = (generationId) => {
    if (!generationId) return null;
    try {
      const raw = localStorage.getItem(QUIZ_MAP_KEY);
      const map = raw ? JSON.parse(raw) : {};
      return map[generationId] || null;
    } catch (e) {
      return null;
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
        ...
        const gen = await api.generations.create({
            ...
            subject: subject || "Test", topic: topic, details: "", grade: grade, duration: 45, lang, prompt: promptText, status: 'running'
        });

        const streamResult = await api.generate.stream(promptText);
        const accumulatedText = streamResult?.markdown || '';

        const generationId = gen.id || gen.generationId;
        await api.generations.update(generationId, { status: "done", result_md: accumulatedText, result_json: accumulatedText });
        const newTest = { id: generationId, result_md: accumulatedText, result_json: accumulatedText, topic, subject, accessCode: null };
  ...
  const handleStartSession = async () => {
     ...
     try {
         const data = await api.quiz.start({ userId: api.getUserId(), topic: generatedTest.topic || topic, generationId: generatedTest.id, questionsJson: generatedTest.result_json || generatedTest.result_md, ttlSeconds: 1800 });
         const quizId = data.data?.quizId || data.quizId || data.id || null;
         const code = data.data?.accessCode || data.accessCode || data.access_code || data.code;
         if (quizId) {
             saveQuizMapping(generatedTest.id, quizId);
         }
         if (code || quizId) {
             setAccessCode(code || null);
             setGeneratedTest(prev => ({...prev, accessCode: code || prev?.accessCode || null, quizId: quizId || prev?.quizId || null}));
         }
     } catch(e) { console.error(e); alert("Network Error"); }
  };

  const fetchReport = async (forceQuizId = null) => {
    const quizId = forceQuizId || generatedTest?.quizId || readQuizMapping(generatedTest?.id);
    if (!quizId) return;
    if (generatedTest?.id && !generatedTest?.quizId) {
      setGeneratedTest(prev => ({ ...prev, quizId }));
    }
    try {
        const data = await api.quiz.report(quizId);
        setReport(data.data?.results || data.results || data.rows || data.data?.rows || []);
    } catch (e) { console.error(e); }
  };

  const getDetailsSafe = (student) => {
      if (!student) return [];
      const raw = student.answersJson ?? student.answers_json;
      if (!raw) return [];
      try {
  ...
  const handleSelectOldTest = (item) => {
    const resolvedQuizId = item.quizId || readQuizMapping(item.id);
    setAccessCode(item.accessCode || item.access_code || null);
    setReport(null);
    setSelectedStudent(null);
    setGeneratedTest({ ...item, quizId: resolvedQuizId || null });
    if (resolvedQuizId) {
      fetchReport(resolvedQuizId);
    } else {
      alert('Start session to get report');
    }
  };
  ...
  const generateAiReport = async (type) => {
    ...
    try {
        ...
        const summaryData = report.map(r => ({
            ...
            score: `${r.score}/${r.total ?? r.total_questions}`,
            time: `${r.duration ?? r.duration_seconds}s`,
            status: type === 'coach' 
        ...
        const aiResponse = await api.generate.stream(promptSystem);
        const accumulatedText = aiResponse?.markdown || '';
                                ...
                                <tbody className="font-bold text-sm">
                                    {report.map((r, i) => (
                                        <tr key={i} className="border-b border-gray-100 dark:border-zinc-700/50 last:border-0 hover:bg-white dark:hover:bg-zinc-800 transition">
                                            <td className="p-3">{r.studentName ?? r.student_name}</td>
                                            <td className="p-3">
                                                <span className={`px-2 py-1 rounded text-xs text-white ${r.percentage >= 80 ? 'bg-green-500' : r.percentage >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`}>
                                                    {r.score}/{r.total ?? r.total_questions}
                                                </span>
                                            </td>
                                            <td className="p-3 text-gray-500">{r.duration ?? r.duration_seconds}s</td>
                                            <td className="p-3">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in zoom-in duration-200">
            <div className="bg-white dark:bg-zinc-900 w-full max-w-lg max-h-[80vh] overflow-hidden rounded-[30px] shadow-2xl flex flex-col border-[4px] border-black dark:border-gray-700">
                <div className="p-6 border-b border-gray-100 dark:border-zinc-800 flex justify-between items-center bg-gray-50 dark:bg-zinc-800/50">
                    <div>
                        <h3 className="text-xl font-black uppercase">{selectedStudent.studentName ?? selectedStudent.student_name}</h3>
