...
const ClassControlBar = () => {
  ...
  useEffect(() => {
    let active = true;
    api.economy.coins().then((coinsData) => {
      if (!active) return;
      setCoins(coinsData?.data?.coins ?? coinsData?.coins ?? 0);
    }).catch(() => {}).finally(() => {
      ...
      <motion.div drag dragMomentum={false} style={{ position: 'fixed', bottom: '40px', right: '40px', zIndex: 9999 }}>
        {!isExpanded ? (
          ...
          <motion.div layoutId="panel" className="bg-white border-[4px] border-black p-4 rounded-[30px] shadow-[10px_10px_0_0_#000] flex items-center gap-4 text-black">
            ...
            <button onClick={() => setIsPollOpen(true)} className="p-2.5 bg-yellow-400 border-2 border-black rounded-xl hover:-translate-y-1 transition shadow-[2px_2px_0_0_#000]"><FerrisWheel size={20} /></button>
            ...
            <div className="flex items-center gap-2 border-l-2 border-r-2 border-gray-100 px-3">
              <span className="font-mono font-black text-xl italic min-w-[50px]">{Math.floor(seconds / 60)}:{(seconds % 60).toString().padStart(2, '0')}</span>
              <button onClick={() => setIsActive(!isActive)} className={`p-1.5 rounded-lg border-2 border-black ${isActive ? 'bg-red-400' : 'bg-green-400'}`}>{isActive ? <Square size={14} fill="black" /> : <Play size={14} fill="black" />}</button>
            </div>
            <div className="flex items-center gap-2 pr-2"><Trophy size={20} className="text-orange-500 fill-orange-500" /><span className="font-black text-lg">{loadingCoins ? '...' : coins}</span></div>
      ...
      <AnimatePresence>
        {isPollOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[10000] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="relative w-full max-w-6xl h-[85vh] bg-white rounded-[30px] overflow-hidden border-[6px] border-black shadow-2xl">
              <button onClick={() => setIsPollOpen(false)} className="absolute top-4 right-4 z-50 p-2 bg-white rounded-full hover:bg-slate-100 border-2 border-black transition"><X size={24} color="black" /></button>
              <iframe src="https://wheelofnames.com/ru/e4143e6c-adf8-449d-9dce-ef3d8fc48ffb" title="Wheel of Fortune" className="w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
