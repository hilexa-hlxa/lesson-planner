 ...
 *
 * PRO настоящий (лимиты применяются на бэкенде — см. backend/src/Plans.php),
 * но оплата не автоматизирована: кнопка PRO — mailto, апгрейд включается
 * вручную через admin-панель после оплаты вне сайта.
 */
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        {/* ── PRO: настоящий тариф, тот же визуальный вес, что у Бесплатного ── */}
        <div className="rounded-[28px] sm:rounded-[32px] border-[3px] border-black dark:border-white bg-white dark:bg-zinc-900 p-6 sm:p-8 shadow-[8px_8px_0_0_#000] dark:shadow-[8px_8px_0_0_rgba(255,255,255,0.15)]">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-600">{t.pro.name}</span>
            {t.pro.badge && (
              <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 text-[9px] font-black uppercase tracking-widest">
                <Sparkles size={9} /> {t.pro.badge}
              </span>
            )}
          </div>
          <div className="flex items-end gap-2 flex-wrap mb-1">
            <span className="text-4xl sm:text-5xl font-black tracking-tighter">{t.pro.price}</span>
