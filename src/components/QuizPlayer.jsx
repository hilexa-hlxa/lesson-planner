...
const normalizeJsonQuestions = (input) => {
  if (!input) return [];
  ...
  return asArray
    .map((q) => {
      const options = Array.isArray(q?.options)
        ? q.options.map((opt) => (typeof opt === 'string' ? opt : (opt?.text || opt?.label || String(opt || '')))).filter(Boolean)
        : [];

      if (options.length < 2) return null;

      let correctIndex = Number.isInteger(q?.correctIndex) ? q.correctIndex : -1;
      if (correctIndex < 0 && q?.correctOption !== undefined) {
        const idx = options.findIndex((o) => o === q.correctOption);
        if (idx >= 0) correctIndex = idx;
      }
      if (correctIndex < 0 && q?.correctAnswer !== undefined) {
...
const parseQuestions = (payload) => {
  if (!payload) return [];

  if (typeof payload === 'object') {
    return normalizeJsonQuestions(payload);
  }
  ...
  if (typeof payload === 'string') {
    ...
    if (trimmed.startsWith('{') || trimmed.startsWith('[')) {
      try {
        return normalizeJsonQuestions(JSON.parse(trimmed));
      } catch {
        // fallback to markdown parser
      }
    }

    return parseMarkdownQuiz(trimmed);
  }
...
const QuizPlayer = ({ grantAchievement }) => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const questionsPayload = quizData.questionsJson || quizData.questions_json || quizData.result_json || quizData.result_md || quizData.result;
      const parsed = parseQuestions(questionsPayload);
  ...
  const handleFinishQuiz = async () => {
    ...
    try {
        await api.quiz.submit({
            ...
            answersJson: JSON.stringify(answersLog),
            answers: answersLog
        });

        if (percent === 100) {
            await api.achievements.grant('perfect_score').catch(() => {});
        }
    } catch (e) {
